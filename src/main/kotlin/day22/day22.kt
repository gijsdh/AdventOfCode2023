...
//    for ()
    ...
    for (obj in objects) {
        ...
        work.removeAll(obj)
        var bad =  false;
        for (cubes in objects) {
            if (obj == cubes) continue
            var moveDown = true
            for (cube in cubes) {
                if (cube.third == 1) moveDown = false
                val shiftDown = Triple(cube.first, cube.second, cube.third - 1)
                if (work.contains(shiftDown) && !cubes.contains(shiftDown)) {
                    moveDown = false
                }
            }
...
fun toTriple(strings: List<String>): Triple<Int, Int, Int> {

    return Triple(strings[0].toInt(), strings[1].toInt(), strings[2].toInt())
}
